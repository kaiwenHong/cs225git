...
NimLearner::NimLearner(unsigned startingTokens) : g_(true, true) {
    ...
    for (unsigned int i = 0; i <= startingTokens; i++) {
      Vertex vtx1 = "p1-" +std::to_string(i);
      Vertex vtx2 = "p2-" +std::to_string(i);
      g_.insertVertex(vtx1);
      g_.insertVertex(vtx2);
    }

    //set up edes between vertex
...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 while (current != "p1-0" && current != "p2-0") {
   int index = std::rand() % g_.getAdjacent(current).size();
   Vertex newVertex = g_.getAdjacent(current)[index];
   path.push_back(g_.getEdge(current, newVertex));
   current = g_.getAdjacent(current)[index];
 }
  return path;
}
