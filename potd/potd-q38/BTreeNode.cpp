...
bool underflows(unsigned numElem, unsigned order) {
    // TODO: Your code here!
    if (numElem < 1 || numElem < ceil((order - 1) / 2)) return true;
    return false;
}
...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
    ...
    traversal.push_back(this->elements_[idx-1]);
    for (unsigned i = 0; i < children_[idx]->elements_.size(); i++) {
      traversal.push_back(children_[idx]->elements_[i]);
    }
    children_[idx]->elements_.swap(traversal);
    this->elements_[idx-1] = children_[idx-1]->elements_.back();
    children_[idx-1]->elements_.pop_back();
    return true;
}
