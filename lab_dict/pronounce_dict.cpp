...
{
    /* Your code goes here! */
    string first_word = word1;
    string second_word = word2;
    std::transform(first_word.begin(), first_word.end(), first_word.begin(), ::toupper);
    std::transform(second_word.begin(), second_word.end(), second_word.begin(), ::toupper);

    //Return false if word does not exist in dictionary.
    if (dict.find(first_word) == dict.end()) return false;
    if (dict.find(second_word) == dict.end()) return false;

    //Return false if values of two words are differ.
    if (dict.at(first_word).size() != dict.at(second_word).size()) return false;

    //Return false if any of the pronunciations differ.
    for (size_t i = 0; i < dict.at(first_word).size(); i++) {
        if (dict.at(first_word).at(i) != dict.at(second_word).at(i)) return false;
    }
