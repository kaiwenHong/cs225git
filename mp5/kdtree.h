...
class KDTree
{
  ...
  public:
     ...
     * total number: a 3KDTreeNode *D tree will have levels split by dimension 0, 1, 2,
     * 0, 1, 2, etc.
     *
  ...
  private:
     ...
     */
     ...
     /**
     * Helper private-variable to keep track of all the points in the tree
     **/
     vector<Point<Dim>> points;
     /**
     * Creates a kd-tree
     * @param left The leftmost index of the points inside the subtree
     *    right The rightmost index of the points inside the subtree
     *    dimension The current dimension we are comparing in respect to
     * @return The new subroot that is created with the median in the range.
     **/
     KDTreeNode * makeTree(int left, int right, int dimension);
     /**
     * Swaps the elements at index1 and index1 of our kd-tree's points.
     * @param index1 The first index we want to swap with
     *    index2 The second index we want to swap with
     **/
     void swap(int index1, int index2);
     /**
     * Rearranges our kd-tree's points, move all smaller points to the left of the pivot and
     *    all larger points to the right of the pivot.
     * @param left The leftmost index of the range we want to rearrange
     *    right The rightmost index of the range we want to rearrange
     *    pivotIndex The index of the point we want to compare the rest of the points to
     *    dimension The current dimension we are comparing in respect to
     * @return The new index of the pivot value
     **/
     int partition(int left, int right, int pivotIndex, int dimension);
     /**
     * Finds the kth smallest value in a given range; in this case, it finds the median.
     * @param left The leftmost index of the range we're selecting from
     *    right The rightmost index of the range we're selecting from
     *    k The kth smallest value we want to find; in our case, the median index of the vector
     *    dimension The current dimension we are comparing in respect to
     **/
     void select(int left, int right, int k, int dimension);
     /**
     * Frees memory associated with a subtree
     * @param subroot The root of the subtree we want to delete
     **/
     void clean(KDTreeNode *subroot);
     /**
     * Calculates the distance between two points
     * @param first The first point we want to find the distance between
     *    second The second point we want to find the distance between
     **/
     double distance(const Point<Dim> &first, const Point<Dim> &second) const;
     /**
     * Finds the nearest neighbor
     * @param subroot The root of the subtree we're currently searching in
     *    target The target point
     *    currentBest The closest neighbor we have found so far
     *    dimension The current dimension we are comparing in respect to
     * @return The closest point in the subtree to the target point
     **/
     Point<Dim> findNearest(KDTreeNode *subroot, const Point<Dim> target, Point<Dim> currentBest, int dimension) const;
};
