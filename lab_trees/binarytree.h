...
class BinaryTree
{
    ...
    private:
         ...
         * @param subRoot The current node in the recursion
         */
        void sumDistances(const Node* subRoot, unsigned& sum, unsigned distance) const;

        /**
         * Private helper function for the public getPaths function.
         * @param subRoot The current node in the recursion
         */
        void getPaths(vector<vector<T> > &paths, vector<T> temp, const Node* subRoot) const;
